...
@torch.no_grad()
def load_quant_params(qnn, ckpt_path, dtype=torch.float32, from_int=False):
    ...
    if from_int:
        qnn.load_int_weight(ckpt, module=qnn.model, dtype=dtype)
    else:
...
def quantize_per_tensor_uint4(input: torch.Tensor):
    int_repr = input
