...
from qdiff.qcnn_hardware.models.quant_block import get_specials, BaseQuantBlock
from qdiff.qcnn_hardware.models.quant_block import QuantTransformerBlock, QuantResnetBlock2D
from qdiff.qcnn_hardware.models.quant_layer import QuantLayer
from qdiff.qcnn_hardware.quantizer.base_quantizer import StraightThrough, BaseQuantizer, WeightQuantizer, ActQuantizer
...
class QuantModel(nn.Module):
    def __init__(self, model: nn.Module, weight_quant_params: dict = {}, act_quant_params: dict = {}, **kwargs):
        ...
        self.model = model
        # self.model = copy.deepcopy(model)  # 创建一个 model 的副本
        # self.in_channels = model.in_channels
        if hasattr(model, 'image_size'):
            self.image_size = model.image_size
        self.specials = get_specials()  # some nn.Modules require special process
        # logger.info(f"\n --------------- refactoring quant layers --------------- \n")
        self.quant_layer_refactor(self.model, weight_quant_params, act_quant_params, save_int=kwargs.get("save_int", False), from_int=kwargs.get("from_int", False))
    ...
    def quant_layer_refactor(self, module: nn.Module, weight_quant_params: dict = {}, act_quant_params: dict = {}, save_int: bool=False, from_int: bool=False):
        """
        ...
        for name, child_module in module.named_children():
            if isinstance(child_module, (nn.Conv2d, nn.Conv1d, nn.Linear)):
                ...
                setattr(module, name, QuantLayer(
                    child_module, weight_quant_params, act_quant_params, save_int=save_int, from_int=from_int))
                prev_quantmodule = getattr(module, name)
                # logger.info(f"\n origional module: {name}:{tmp_module}, \n new module {prev_quantmodule}")
            elif isinstance(child_module, StraightThrough):
                continue
            else:
                self.quant_layer_refactor(child_module, weight_quant_params, act_quant_params, save_int=save_int, from_int=from_int)  # recursive call1
    ...
    def load_int_weight(self, ckpt, module=None, dtype=torch.float32, prefix=""):
        if module is None:
            module = self.model
        ...
        for name, module_ in module.named_children():
            ...
            if isinstance(module_, QuantLayer):
               # unpack the dict
                setattr(module_, 'weight', ckpt[full_name+'.weight'])
                if module_.bias:
                    setattr(module_, 'bias', ckpt[full_name+'.bias'])

                weight_quantizer = module_.weight_quantizer
                setattr(weight_quantizer, 'delta', ckpt[full_name+'.weight_quantizer.delta'].to(dtype))
                setattr(weight_quantizer, 'delta_list', ckpt[full_name+'.weight_quantizer.delta_list'].to(dtype))
                setattr(weight_quantizer, 'zero_point', ckpt[full_name+'.weight_quantizer.zero_point'].to(dtype))
                setattr(weight_quantizer, 'zero_point_list', ckpt[full_name+'.weight_quantizer.zero_point_list'].to(dtype))
            else:
                self.load_int_weight(ckpt=ckpt, module=module_, prefix=full_name+".")
