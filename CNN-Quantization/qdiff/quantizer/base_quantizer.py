...
def lp_loss(pred, tgt, p=2, reduction='none', n_batch=None):
    ...
    with open('/root/yixiaojie/CNN-Quantization/debug/out_quant_pred.txt', 'w') as f:
        f.write(str(pred))
    with open('/root/yixiaojie/CNN-Quantization/debug/out_quant_tgt.txt', 'w') as f:
        f.write(str(tgt))
    # print(len(pred))
    # print(pred[0])
    # print(len(pred[0]))
...
def lp_loss_yolo(pred, tgt, p=2, reduction='none', n_batch=None):
    """
    loss function measured in L_p Norm for comparing `boxes.cls`, `boxes.conf`, `boxes.data`
    """
    
    # Ensure pred and tgt are tensors
    assert pred.shape == tgt.shape, f"Shape mismatch: pred.shape {pred.shape} vs tgt.shape {tgt.shape}"

    # Extract cls, conf, and data from the `boxes` object
    pred_boxes = pred.boxes
    tgt_boxes = tgt.boxes
    
    pred_cls = pred_boxes.cls  # (N, num_boxes)
    tgt_cls = tgt_boxes.cls
    pred_conf = pred_boxes.conf  # (N, num_boxes)
    tgt_conf = tgt_boxes.conf
    pred_data = pred_boxes.data  # (N, num_boxes, 6)
    tgt_data = tgt_boxes.data

    # Initialize the MSE for each part
    mse_cls = mse_conf = mse_data = 0.0
    
    # Loop through each image in the batch
    for i in range(pred.shape[0]):  # Assuming batch size is the first dimension
        pred_cls_img = pred_cls[i]
        tgt_cls_img = tgt_cls[i]
        pred_conf_img = pred_conf[i]
        tgt_conf_img = tgt_conf[i]
        pred_data_img = pred_data[i]
        tgt_data_img = tgt_data[i]
        
        # Calculate MSE for cls, conf, and data
        mse_cls_img = torch.mean((pred_cls_img - tgt_cls_img) ** 2)
        mse_conf_img = torch.mean((pred_conf_img - tgt_conf_img) ** 2)
        mse_data_img = torch.mean((pred_data_img - tgt_data_img) ** 2)
        
        # Accumulate MSE for the whole batch
        mse_cls += mse_cls_img
        mse_conf += mse_conf_img
        mse_data += mse_data_img

    # Normalize by the batch size if reduction is not 'none'
    if reduction != 'none':
        mse_cls /= pred.shape[0]
        mse_conf /= pred.shape[0]
        mse_data /= pred.shape[0]

    # Return a dictionary containing MSE values for each part
    return {'mse_cls': mse_cls, 'mse_conf': mse_conf, 'mse_data': mse_data}